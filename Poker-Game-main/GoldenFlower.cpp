...
bool GoldenFlowerWindow::eventFilter(QObject *watched, QEvent *event) {
    ...
    if (watched == this) {
        ...
        if (event->type() == QEvent::Resize) {
            ...
            // 关键修改：将玩家信息和扑克牌与交点距离也应用缩放因子
            // 这样当窗口缩放时，玩家信息和扑克牌与牌桌边缘的相对位置关系保持不变
            // 使用原始距离值(50)乘以缩放因子，确保距离随窗口大小等比例变化
            int newPlayerInfoDistance = int(50 * newScaleFactor); // 应用缩放因子到玩家信息距离
            int newCardDistance = int(50 * newScaleFactor);       // 应用缩放因子到卡牌距离
...
void GoldenFlowerWindow::adjustLayoutParameters(int newTableWidth, int newTableHeight, int newPlayerInfoDistance, int newCardDistance, float newScaleFactor) {
    ...
    // 更新玩家信息和扑克牌与交点的距离，保持比例不变
    // 点击操作页面的最大化按钮之后，仍需要计算玩家信息和扑克牌距离桌子的距离，保持不变
    // 这里我们使用缩放因子来确保距离保持相对一致
    // 关键点：playerInfoDistance和cardDistance会随着窗口大小变化而等比例变化
    // 这确保了在窗口最大化或其他尺寸变化时，玩家信息和扑克牌与交点的相对距离保持不变
    playerInfoDistance = newPlayerInfoDistance;
    cardDistance = newCardDistance;
...
void GoldenFlowerWindow::showComparisonDialog(int player1Index, int player2Index) {
    Player& player1 = players[player1Index];  // 获取第一个玩家引用
    Player& player2 = players[player2Index];  // 获取第二个玩家引用
    ...
    // 显示第一个玩家的牌
    QGroupBox* player1Box = new QGroupBox(QString::fromStdString(player1.name));  // 创建第一个玩家分组框
    QVBoxLayout* player1Layout = new QVBoxLayout(player1Box);  // 创建垂直布局用于第一个玩家区域
    
    QHBoxLayout* player1CardLayout = new QHBoxLayout();  // 创建水平布局用于显示第一个玩家的卡牌
    player1CardLayout->setAlignment(Qt::AlignCenter);    // 设置卡牌居中对齐
    player1CardLayout->setSpacing(10);                   // 设置卡牌间距
...
void GoldenFlowerWindow::updateUI() {
    ...
    for (int i = 0; i < numPlayers; ++i) {
        ...
        // 计算玩家信息位置（交点外侧延长线上）
        infoX = markerX + dx * playerInfoDistance;
        infoY = markerY + dy * playerInfoDistance;
        
        // 计算扑克牌位置（交点内侧延长线上）
        cardX = markerX - dx * cardDistance;
        cardY = markerY - dy * cardDistance;
        ...
        if (distance > 0.0) {
            ...
            // 关键修改：确保玩家信息和扑克牌分别位于交点两侧
            // 玩家信息位于交点外侧（远离中心）
            // 使用playerInfoDistance作为基准距离，该值已经在窗口大小变化时通过缩放因子进行了调整
            // 这确保了无论窗口如何变化（包括最大化），玩家信息与交点的距离始终保持相对一致
            infoX = markerX + nx * playerInfoDistance;
            infoY = markerY + ny * playerInfoDistance;
            
            // 计算卡牌容器的尺寸（3张牌加间距）
            // 确保所有尺寸都与缩放因子成正比，这样在窗口最大化时元素会保持正确的比例
            int cardContainerWidth = (cardWidth * 3 + cardSpacing * 2); // 3张牌的总宽度加间距
            int cardContainerHeight = cardHeight; // 容器高度等于卡牌高度
            
            // 扑克牌位于交点内侧（靠近中心）
            // 注意：这里使用负方向，确保卡牌位于交点与中心点之间
            // 同样使用cardDistance作为基准距离，该值已经在窗口大小变化时通过缩放因子进行了调整
            // 这确保了无论窗口如何变化（包括最大化），扑克牌与交点的距离始终保持相对一致
            // 修改：使用完整的cardDistance作为距离，确保卡牌与交点距离完全保持一致
    ...
    if (activePlayers == 2) {
        ...
        if (ret == QMessageBox::Ok) {
            ...
            if (currentPlayerWins) {
                ...
                QMessageBox::information(this, "比牌结果", 
                                      QString::fromStdString(players[targetPlayerIndex].name) + " 输牌，已弃牌。");
            } else {
                // 当前玩家输，设置为弃牌状态
                currentPlayer.status = PlayerStatus::FOLDED;
                QMessageBox::information(this, "比牌结果", 
                                      QString::fromStdString(currentPlayer.name) + " 输牌，已弃牌。");
            }
        ...
        // 显示消息通知玩家
        QMessageBox::information(this, "请求开牌", 
                              QString::fromStdString(currentPlayer.name) + " 请求与 " + 
                              QString::fromStdString(players[targetPlayerIndex].name) + " 开牌，轮到下家说话。");
    }
}
